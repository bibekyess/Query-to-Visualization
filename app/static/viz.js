...
(function () {
  ...
  function toVegaLite(viz) {
    ...
    if (type === 'scatter') {
      ...
      return {
        ...
        title,
        mark: { type: 'point', filled: true, opacity: 0.6, color: '#c05e3c' },
        data: { values: data },
        encoding: {
    ...
    return {
      ...
      width: 'container',
      height: 340,
      config: { range: { category: ['#c05e3c', '#3c7a89', '#c9a227', '#6b7280', '#8b5a8c', '#4a7c59'] } },
    };
  }
  ...
  function renderNetwork(container, viz) {
    ...
    new vis.Network(container, { nodes, edges }, {
      nodes: { scaling: { min: 10, max: 46 }, font: { size: 12 }, borderWidth: 1, color: { background: '#e8ddd0', border: '#c05e3c', highlight: { background: '#c05e3c', border: '#8f4227' } } },
      edges: { scaling: { min: 1, max: 8 }, color: { color: '#c4b5a5', opacity: 0.7 }, smooth: { type: 'continuous' } },
